...
# lazy as fuck, gonna define these globally
ncol <- 4000 
nrow <- 500
...
subdivision <- function(nsplits = 50, 
                        seed = NULL) {
...
develop <- function(div, seed = NULL, linewidth = 3) {
  ...
  poly <- list()
  base_x <- cos(seq(0, 2*pi, length.out = 100)) * nrow / ncol
  base_y <- sin(seq(0, 2*pi, length.out = 100))
  
  for(i in 1:24) {
    
    radius <- rbeta(1, 1, 1) * .25
    x_shift <- runif(1, min = 0, max = 1)
    y_shift <- runif(1, min = .03, max = 1.3)
  ...
  div |> 
    ...
    coord_equal(
      xlim = c(0, ncol * pct_clip) + (ncol - (ncol * pct_clip))/2, 
      ylim = c(0, nrow * pct_clip) + (nrow - (nrow * pct_clip))/2 + c(-60, 260)
    ) +
    theme_void() + 
...
write_subdivision <- function(seed) {
  fname <- paste0("subdivision_08_", seed, ".png")
