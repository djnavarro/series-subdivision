...
develop <- function(div, seed = NULL, linewidth = 3) {
  ...
  poly <- list()
  for(i in 1:12) {
    xval <- runif(3, min = -.5, max = 1.5)
    yval <- runif(3, min = -.5, max = 1.5)
  ...
  div |> 
    ...
    coord_equal(xlim = c(10, ncol), ylim = c(10, nrow)) +
    theme_void() + 
...
write_subdivision <- function(seed) {
  fname <- paste0("subdivision_05_", seed, ".png")
  cat("generating", fname, "\n")
  subdivision(seed = seed, nsplits = 300) |> 
    develop(linewidth = 1) |> 
    ggsave(
